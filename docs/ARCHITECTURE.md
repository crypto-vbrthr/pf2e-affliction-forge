...
## Restriction runtime and residual output (0.1.49)
...
## Component persistence and typed healing locks (0.1.51)

Persistent stage output is no longer forced to share one lifetime. `affliction-instance-service.js` resolves each persistent Critical Forge component against `stage.effectComponentPersistence[index]`; absent/`null` entries inherit the stage-wide `effectPersistence`. Components with different effective lifetimes are compiled into separate managed PF2E Effect Items, allowing one consequence to remain permanent while its siblings still disappear on stage transition.
